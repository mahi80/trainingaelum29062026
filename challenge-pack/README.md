...
|----------|---------------|----------|-----------|
| **Scanned documents (OCR)** | `example/images/*.jpg` (100) + `example/gt/` | **9 doc types** (application, verification, policy, pay stub, W-2, dealer invoice, driver license, vehicle title, insurance card); multi-page tables, **merged/spanning cells**, skew/perspective, cursive handwriting, **stamps/watermarks/redaction/checkboxes**, scan artifacts, a few 90°/180° pages. GT per page: `cells.json` (grid), `tables.html` (rowspan/colspan), `hocr`, `alto.xml`. **80 train / 20 test** (test GT withheld). | `eval/ocr_eval.py` (TEDS/GriTS/CER) |
